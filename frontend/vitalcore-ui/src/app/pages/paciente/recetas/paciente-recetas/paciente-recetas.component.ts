...
@Component({
  ...
  styleUrl: './paciente-recetas.component.scss'
})
  ...
  ngOnInit(): void {
    ...
    this.recetasService.getAll().subscribe({
      next: (all) => {
        ...
        // Filter: match receta.tratamiento.cita.paciente.idPaciente == uid
        this.recetas = all.filter(r =>
          r.tratamiento &&
          r.tratamiento.cita &&
          r.tratamiento.cita.paciente &&
          r.tratamiento.cita.paciente.idPaciente === uid
        );
        this.loading = false;
      },
      error: (e) => {
